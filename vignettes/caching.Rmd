...
### Caching on a Function by Function Basis

Caching can be enabled two ways.
The most simple and direct is to set `cache = TRUE` in the function arguments that support caching.
This will override any global settings for that function only.
For example, you might wish to only cache the national land use data, but not the AAGIS Regions data.

```r 
nlum <- read_nlum_stars(data_set = "Y202021", cache = TRUE)
aagis_regions <- read_aagis_regions(cache = FALSE)
```


### Enabling Caching and Setting the Location Globally
