skip_if_not("read_soil_thickness_stars" %in% ls(getNamespace("read.abares")))
...
test_that("soil_thickness: happy path with stars::read_stars mocked", {
  ...
  testthat::local_mocked_bindings(read_stars = function(...) build_dummy_stars(), .package = "stars")

  out <- read_soil_thickness_stars(files)
...
test_that("soil_thickness: wrong class for files errors cleanly", {
  ...
  bad <- c("a.tif", "b.tif")
  expect_error(read_soil_thickness_stars(bad))
})
