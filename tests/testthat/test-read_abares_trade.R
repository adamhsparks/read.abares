...
call_trade <- function(...) if (has_read) read_abares_trade(...) else get_abares_trade(...)

# Header that matches the package's renaming logic
.make_trade_csv_text <- function() {
  header <- c(
    "Fiscal_year","Month","YearMonth","Calendar_year","TradeCode",
    "Overseas_location","State","Australian_port","Unit",
    "TradeFlow","ModeOfTransport","Value","Quantity","confidentiality_flag"
  )
  rows <- list(
    c("2023-24","Jan","2023-01","2023","0101","NZ","WA","Fremantle","t","Export","Sea","1000","10",""),
    c("2023-24","Feb","2023-02","2023","0101","US","WA","Fremantle","t","Import","Air","2000","20","")
  )
  paste(
...
test_that("Trade: download error propagates", {
  testthat::local_mocked_bindings(.retry_download = function(...) stop("timeout"))
