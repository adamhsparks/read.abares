...
call_aagis <- function(...) if (has_read) read_aagis_regions(...) else get_aagis_regions(...)

# Valid sf for tests (geometry present)
fake_sf <- function(n = 2L) {
  xs <- 115 + seq_len(n) * 0.01
  ys <- -31 - seq_len(n) * 0.01
  pts <- sf::st_sfc(lapply(seq_len(n), \(i) sf::st_point(c(xs[i], ys[i]))), crs = 4326)
...
test_that("AAGIS: provided path => unzip + read mocked", {
  ...
  tmp_zip <- withr::local_tempfile(fileext = ".zip"); writeLines("zip", tmp_zip)
  ...
  testthat::local_mocked_bindings(file_delete = function(x) invisible(x), .package = "fs")
  testthat::local_mocked_bindings(dir_delete  = function(x) invisible(x), .package = "fs")
  testthat::local_mocked_bindings(st_read     = function(dsn, ...) fake_sf(), .package = "sf")
...
test_that("AAGIS: x=NULL => mocked download + unzip + read", {
  ...
  testthat::local_mocked_bindings(
    .retry_download = function(url, .f) { writeLines("zip", .f); invisible(NULL) },
    .unzip_file     = function(x) exdir
  )
  testthat::local_mocked_bindings(dir_ls      = function(...) shp, .package = "fs")
  testthat::local_mocked_bindings(file_delete = function(x) invisible(x), .package = "fs")
...
test_that("AAGIS: controlled failure at read", {
  ...
  testthat::local_mocked_bindings(file_delete = function(x) invisible(x), .package = "fs")
  testthat::local_mocked_bindings(dir_delete  = function(x) invisible(x), .package = "fs")
  testthat::local_mocked_bindings(st_read     = function(...) stop("network fail"), .package = "sf")
